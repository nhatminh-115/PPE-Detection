...
from src.infrastructure.supabase import get_supabase_client, get_supabase_service_client
...
def _run_backfill_if_needed() -> None:
    """
    Run model-metadata backfill once at startup if any ppe_labels rows have null
    model_name. Runs in a background thread so it does not delay server startup.
    """
    try:
        ...
        null_count = (res.count or 0) if hasattr(res, "count") else len(res.data or [])
        if null_count == 0:
            return
        logger.info("Backfilling model metadata for %d ppe_labels row(s)...", null_count)
        result = backfill_model_metadata()
        logger.info("Backfill complete: %s", result)
    except Exception as exc:
        logger.debug("Startup backfill skipped: %s", exc)


@app.on_event("startup")
def startup_tasks() -> None:
    threading.Thread(target=_run_backfill_if_needed, daemon=True).start()
...
def get_router_config():
    return {"min_side_px": float(_clf.router_min_side_px)}
...
def update_router_config(min_side_px: float = 110.0):
    ...
    return {"status": "success", "router": {"min_side_px": clamped}}
...
def get_all_violations(limit: int = 500, session_id: str = None):
    """Full violation log for labeling interface. Hard cap at 2000."""
    client = get_supabase_client()
    if not client:
        return {"data": [], "total": 0}

    safe_limit = min(int(limit), 2000)
    query = client.table("ppe_violations").select("*")
    if session_id:
        query = query.eq("session_id", session_id)

    try:
        rows = query.order("created_at", desc=True).limit(safe_limit).execute().data or []
        return {"data": rows, "total": len(rows)}
    except (SupabaseAPIError, Exception) as exc:
        logger.warning(f"Supabase unavailable (violations/all): {exc}")
        return {"data": [], "total": 0}


@app.post("/api/second_opinion/run")
async def run_second_opinion(date: str = None):
    """
    Run the Phase 3 second opinion batch for a given ICT date.
    Finds uncertain violation events (avg conf < SECOND_OPINION_THRESHOLD),
    sends each crop to Llama 4 Scout, stores auto-labels in ppe_labels.
    Human-labeled events are never overwritten.

    Query param: date (YYYY-MM-DD, ICT). Defaults to today ICT.
    """
    from src.api.second_opinion import run_second_opinion_batch
    try:
        result = await run_second_opinion_batch(date)
        return result
    except Exception as exc:
        logger.error("Second opinion batch failed: %s", exc)
...
async def generate_report(date: str = None):
    """
    Generate a daily PPE safety report.

    Uses the LangGraph multi-agent pipeline when REPORT_USE_LANGGRAPH=1,
    otherwise falls back to the legacy report_service path.
    ...
    use_langgraph = os.environ.get("REPORT_USE_LANGGRAPH", "0").strip() == "1"

    if use_langgraph:
        try:
            from src.api.report_pipeline import run_report_pipeline
            result = await run_report_pipeline(date)
            return {
                "engine":       "langgraph",
                "date":         date,
                "narrative":    result.get("narrative", ""),
                "summary":      result.get("summary", {}),
                "systemic_flags": result.get("systemic_flags", []),
                "quality":      result.get("quality", {}),
            }
        except Exception as exc:
            logger.error("LangGraph report pipeline failed: %s", exc)
            from fastapi import HTTPException
            raise HTTPException(status_code=500, detail=str(exc))
    else:
        # Legacy path — report_service operates on today's date only
        try:
            import report_service
            from datetime import date as _date
            supabase = report_service.get_supabase()
            groq     = report_service.get_groq()
            report_date = _date.fromisoformat(date)
            rows = report_service.fetch_today_violations(supabase, report_date)
            if not rows:
                return {"engine": "legacy", "date": date, "narrative": "No violations found.", "summary": {}}
            summary = report_service.aggregate(rows)
            violation_types = list({v["violation_type"] for row in rows for v in row.get("violations", [])})
            reg_context = report_service.fetch_regulation_context(violation_types)
            narrative = report_service.generate_narrative(groq, summary, report_date, reg_context)
            report_service.save_to_supabase(supabase, report_date, summary, narrative)
            return {"engine": "legacy", "date": date, "narrative": narrative, "summary": summary}
        except Exception as exc:
            logger.error("Legacy report generation failed: %s", exc)
            from fastapi import HTTPException
            raise HTTPException(status_code=500, detail=str(exc))
...
def download_report_pdf(session_id: str = None):
    ...
    with PdfPages(buf) as pdf:
        ...
        hdr.axis('off')
        hdr.text(0.030, 0.66, 'PPE VISION', fontsize=15, fontweight='bold', color='white', va='center', transform=hdr.transAxes)
        hdr.text(0.030, 0.24, 'SAFETY COMPLIANCE REPORT', fontsize=8, color='#8b949e', va='center', transform=hdr.transAxes)
        session_label = f"Session: {session_id}" if session_id else "All Sessions"
        hdr.text(0.972, 0.66, f"Generated: {now.strftime('%d %b %Y, %H:%M')} ICT", fontsize=8, color='#8b949e', va='center', ha='right', transform=hdr.transAxes)
        hdr.text(0.972, 0.24, f"{session_label}  ·  {total} records", fontsize=8, color='#8b949e', va='center', ha='right', transform=hdr.transAxes)
        ...
        for i, (lbl, val, clr, bg) in enumerate(kpis):
            ...
            ca.set_facecolor(bg)
            ca.axis('off')
            rect = mpatches.FancyBboxPatch((0.01, 0.01), 0.98, 0.98, boxstyle="round,pad=0.01", linewidth=0.6, edgecolor=BORDER, facecolor='none', transform=ca.transAxes, clip_on=False)
            ca.add_patch(rect)
            ca.text(0.50, 0.64, val, fontsize=17, fontweight='bold', color=clr, ha='center', va='center', transform=ca.transAxes)
            ca.text(0.50, 0.20, lbl, fontsize=6.5, color=TXT_M, ha='center', va='center', transform=ca.transAxes)

        ha   = fig.add_axes([0.055, 0.580, 0.555, 0.235])
        ha.set_facecolor('#f6f8fa')
        vals = [hourly.get(h, 0) for h in range(24)]
        bar_clrs = [WARNING if h == peak_hour else (DANGER if v > 0 else '#d0d7de') for h, v in enumerate(vals)]
        ...
        ha.set_xticklabels(['00:00', '06:00', '12:00', '18:00', '23:00'], fontsize=7, color=TXT_M)
        ha.tick_params(axis='y', labelsize=7, colors=TXT_M)
        ha.grid(axis='y', color=BORDER, linewidth=0.5, zorder=0)
        ha.set_axisbelow(True)
        for sp in ['top', 'right']: ha.spines[sp].set_visible(False)
        ha.spines['left'].set_color(BORDER)
        ha.spines['bottom'].set_color(BORDER)
        ha.set_title('Violations by Hour (ICT)', fontsize=9, fontweight='600', color=TXT_D, pad=6, loc='left')
        ha.legend(handles=[mpatches.Patch(color=DANGER, label='Violations'), mpatches.Patch(color=WARNING, label='Peak hour')], fontsize=7, loc='upper right', framealpha=0.9, edgecolor=BORDER)
        ...
        if total_types > 0:
            da.pie([hardhat_count, vest_count], colors=[DANGER, WARNING], startangle=90, wedgeprops=dict(width=0.46))
            da.text(0, 0, str(total_types), ha='center', va='center', fontsize=13, fontweight='bold', color=TXT_D)
        else:
            da.pie([1], colors=['#d0d7de'], wedgeprops=dict(width=0.46))
            da.text(0, 0, '0', ha='center', va='center', fontsize=13, fontweight='bold', color=TXT_M)
        da.set_title('Violation Type', fontsize=9, fontweight='600', color=TXT_D, pad=6, loc='left')
        ...
        la.axis('off')
        la.text(0.00, 0.75, '●', color=DANGER,   fontsize=9, transform=la.transAxes, va='center')
        la.text(0.10, 0.75, f'No Hardhat — {hardhat_count} ({hh_pct})', fontsize=7, color=TXT_M, transform=la.transAxes, va='center')
        la.text(0.00, 0.20, '●', color=WARNING,  fontsize=9, transform=la.transAxes, va='center')
        la.text(0.10, 0.20, f'No Vest — {vest_count} ({vt_pct})', fontsize=7, color=TXT_M, transform=la.transAxes, va='center')
        ...
        if top_offenders:
            ...
            bars_o  = oa.barh(y_pos, counts, color=DANGER, alpha=0.80, height=0.55, edgecolor='none')
            for bar, cnt in zip(bars_o, counts):
                oa.text(bar.get_width() + max(counts) * 0.01, bar.get_y() + bar.get_height() / 2, str(cnt), va='center', fontsize=7, color=TXT_D, fontweight='600')
            oa.set_yticks(y_pos)
            ...
            oa.text(0.5, 0.5, 'No violations recorded', fontsize=9, color=TXT_M, ha='center', va='center', transform=oa.transAxes)
        for sp in ['top', 'right']: oa.spines[sp].set_visible(False)
        oa.spines['left'].set_color(BORDER)
        oa.spines['bottom'].set_color(BORDER)
        oa.set_title('Top Repeat Offenders', fontsize=9, fontweight='600', color=TXT_D, pad=6, loc='left')
        ...
        ta.axis('off')
        ta.set_facecolor(BG)
        ta.text(0.0, 0.985, f'Recent Violation Log  (latest {min(len(rows), MAX_ROWS)} of {total})', fontsize=9, fontweight='600', color=TXT_D, transform=ta.transAxes, va='top')
        ...
        for lbl, x0, x1 in zip(cols, col_x, col_end_x):
            ta.add_patch(mpatches.Rectangle((x0, hdr_y - HDR_H * 0.85), x1 - x0 - 0.003, HDR_H * 0.85, facecolor=HDR_BG, transform=ta.transAxes, clip_on=False))
            ta.text(x0 + 0.006, hdr_y - HDR_H * 0.38, lbl, fontsize=6.5, color='white', fontweight='600', transform=ta.transAxes, va='center')
        ...
        for idx, row in enumerate(table_rows):
            ...
            for x0, x1 in zip(col_x, col_end_x):
                ta.add_patch(mpatches.Rectangle((x0, row_y), x1 - x0 - 0.003, ROW_H * 0.82, facecolor=row_bg, edgecolor=BORDER, linewidth=0.3, transform=ta.transAxes, clip_on=False))
            dt_row   = datetime.fromisoformat(row["created_at"]).astimezone(ICT)
            viols    = row.get("violations", [])
            viol_str = ', '.join(dict.fromkeys(v["violation_type"].replace("none_", "").upper() for v in viols))
            t_id     = str(viols[0]["tracker_id"]) if viols else '—'
            model    = str(row.get("reported_by_model") or (viols[0].get("reported_by_model", "?") if viols else "?")).upper()[:8]
            sess     = str(row.get("session_id", ""))[:8]
            cells    = [str(idx+1), dt_row.strftime('%m/%d %H:%M:%S'), t_id, viol_str, model, sess]
            for val, x0 in zip(cells, col_x):
                txt_clr = DANGER if any(k in val for k in ('HARDHAT', 'VEST')) else TXT_D
                ta.text(x0 + 0.006, row_y + ROW_H * 0.37, str(val)[:22], fontsize=6, color=txt_clr, transform=ta.transAxes, va='center')

        fa = fig.add_axes([0, 0.000, 1, 0.026])
        fa.set_facecolor('#f6f8fa')
        fa.axis('off')
        fa.axhline(y=0.98, color=BORDER, linewidth=0.5)
        fa.text(0.030, 0.50, 'PPE Vision — Operations Center', fontsize=7, color=TXT_M, va='center', transform=fa.transAxes)
        fa.text(0.970, 0.50, f'Generated {now.strftime("%Y-%m-%d %H:%M:%S")} ICT  ·  Page 1 of 1', fontsize=7, color=TXT_M, va='center', ha='right', transform=fa.transAxes)
...
# ---------------------------------------------------------------------------
# Labels  (Phase 1 – Human-in-the-Loop)
# ---------------------------------------------------------------------------
...
class LabelPayload(BaseModel):
    ...
    human_missing:      list[str] | None = None
...
def save_label(payload: LabelPayload):
    """Upsert a human annotation and trigger crop upload if image_path is provided."""
    try:
        saved = upsert_label(payload.model_dump(), labeled_by=payload.labeled_by)
        return {"status": "success", "data": saved}
    except Exception as exc:
        logger.warning(f"save_label failed: {exc}")
...
def get_labels(
    ...
    """List human annotations. Filter by aggregate verdict or session."""
    rows = list_labels(aggregate=aggregate, session_id=session_id, limit=limit)
    return {"data": rows, "total": len(rows)}


@app.get("/api/labels/daily_quality")
def daily_quality(date: str = None):
    """Daily precision/recall aggregation for Phase 2 Data Agent. date=YYYY-MM-DD (ICT)."""
    return get_daily_quality(date_str=date)


@app.post("/api/labels/backfill_metadata")
def backfill_labels_metadata():
    """
    One-time backfill: resolve model_name/model_version/run_id for ppe_labels rows
...
def flush_images():
    ...
    try:
        count = sum(
            1 for f in os.listdir("violation_crops")
            if os.path.isfile(os.path.join("violation_crops", f))
            and not os.remove(os.path.join("violation_crops", f))
        )
